...
void CapturePort::AddCaptureEvent(CaptureEventRef eventRef)
{
	...
	if (eventRef->m_type == CaptureEvent::EtStart)
	{
		...
		LOG4CXX_INFO(s_log, "[" + m_audioTapeRef->m_trackingId + "] #" + m_id + " start  rtpport " + m_audioTapeRef->m_portId + " -- " + m_audioTapeRef->m_localRtpPort+ " -- " + m_audioTapeRef->m_remoteRtpPort );
		logMsg.Format("--------------- MIKI DETAILS CAPTURE EVENT EtStart ----- PORT == " + m_audioTapeRef->m_portId + " -- " + m_audioTapeRef->m_localRtpPort+ " -- " + m_audioTapeRef->m_remoteRtpPort );
		LOG4CXX_INFO(s_log, logMsg);
	}
	...
	else
	{
		...
		{
			logMsg.Format("--------------- CAPTURE EVENT EtEndMetadata ----- " + audioTapeRef->m_remoteRtpPort + " -- " + audioTapeRef->m_localRtpPort );
			LOG4CXX_INFO(s_log, logMsg);
		...
		default:			
			logMsg.Format("--------------- MIKI - CAPTURE EVENT DEFAULT -----" + audioTapeRef->m_localRtpPort + " -- " + audioTapeRef->m_remoteRtpPort );
			LOG4CXX_INFO(s_log, logMsg);
