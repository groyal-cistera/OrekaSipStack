	...
	if(m_singleton.get() == NULL)
	{
		...
		for (std::vector<OrkTrack>::const_iterator it = OrkTrack::getTrackers().begin(); it != OrkTrack::getTrackers().end(); it++) {
			...
			try{
				...
				logMsg.Format("MIKI 1- [%s] failed to start reporting thread reason:%s", rtInfo->m_tracker.ToString(), ex.what());
...
bool Reporting::AddMessage(MessageRef messageRef)
{
	...
	logMsg.Format("MIKI 2- ----------------------  ADD MESSAGED CALLED ---------------");
	LOG4CXX_INFO(LOG.reporting, logMsg);

	IReportable* reportable = dynamic_cast<IReportable*>(messageRef.get());
	if (!reportable) {
		logMsg.Format("MIKI 3- Discarding message that is not reportable: %s", msgAsSingleLineString);
		LOG4CXX_INFO(LOG.reporting, logMsg);
		return false;
	}
	logMsg.Format("MIKI 4- ------- reportable message -------------------");
	LOG4CXX_INFO(LOG.reporting, logMsg);
	...
	for(it = s_reportingThreadsVector.begin(); it != s_reportingThreadsVector.end(); it++)
	{
		...
		logMsg.Format("MIKI 5- [%s] queuesize:%d %s: %s",reportingThread->m_tracker.ToString(),reportingThread->m_messageQueue.numElements(),ret?"enqueued":"queue full, rejected",msgAsSingleLineString);
		LOG4CXX_INFO(LOG.reporting, logMsg);
	}
	logMsg.Format("MIKI 6- ------- go ahead add to queue -------------------");
	LOG4CXX_INFO(LOG.reporting, logMsg);
...
void ReportingThread::Run()
{
	...
	logMsg.Format("MIKI 7- [%s] %sreporting thread started.", m_tracker.ToString(), (m_tracker.m_https? "HTTPS ":""));
	LOG4CXX_INFO(LOG.reporting, logMsg);
	...
	do {
		...
		if (!response.m_success) {
			if (time(NULL) - reportErrorLastTime > 60) {
				logMsg.Format("MIKI 8- [%s] init connection:%s success:false comment:%s ", m_tracker.ToString(), conn?"true":"false", response.m_comment);
				LOG4CXX_INFO(LOG.reporting, logMsg);
	...
	logMsg.Format("MIKI 9- [%s] init success:true comment:%s", m_tracker.ToString(),  response.m_comment);
	LOG4CXX_INFO(LOG.reporting, logMsg);
	...
	for(;stop == false;)
	{
		try
		{
			...
			else
			{
				...
				if (!reportable) {
					logMsg.Format("MIKI 10- [%s] Discarding message that is not reportable:%s", m_tracker.ToString(), msgAsSingleLineString);
					LOG4CXX_INFO(LOG.reporting, logMsg);
					continue;
				}
				...
				{
					logMsg.Format("MIKI 11- [%s] timedeltasecs:%d sending: %s", m_tracker.ToString(), (int)(msgRef->Age()), msgAsSingleLineString);
					LOG4CXX_INFO(LOG.reporting, logMsg);
					...
					while (!success && !IsSkip())
					{
						if (c.Execute((SyncMessage&)(*msgRef.get()), (AsyncMessage&)(*tr.get()), m_tracker.m_hostname, m_tracker.m_port, m_tracker.m_servicename, CONFIG.m_clientTimeout, m_tracker.m_https))
						{
							...
							{
								error = false;
								logMsg.Format("MIKI 12- [%s] successfully reconnected to the tracker after error", m_tracker.ToString());
								LOG4CXX_INFO(LOG.reporting, logMsg);
							}
						...
						else
						{
							...
							if( reportError || ((time(NULL) - reportErrorLastTime) > 60) )	// at worst, one error is reported every minute
							{
								reportError = false;
								reportErrorLastTime = time(NULL);
								logMsg.Format("MIKI 13- [%s] could not connect to tracker", m_tracker.ToString());
								LOG4CXX_INFO(LOG.reporting, logMsg);
							}
		...
		{
			logMsg.Format("MIKI 14- [%s] exception:", m_tracker.ToString(), e);
			LOG4CXX_INFO(LOG.reporting, logMsg);
		}
	}
	logMsg.Format("MIKI 15- [%s] gracefully terminating the reporting thread", m_tracker.ToString());
	LOG4CXX_INFO(LOG.reporting, logMsg);
}
